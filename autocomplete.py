...
class TrieNode:
  ...
  def _preprocess_phrase_counts(self, phrase_long):
    ...
    if phrase in self.stems:
      ...
      self.stems[phrase] = 1
    self.sorted_stems = [(k, self.stems[k]) for k in sorted(self.stems, key=self.stems.__getitem__, reverse=True)]
    if len(phrase) > 1:
  ...
  def add(self, phrase, update_phrase_counts=True):
    ...
    if len(phrase) == 1:
      ...
      self.my_children[phrase[1]].add(phrase[1:], update_phrase_counts=False)
